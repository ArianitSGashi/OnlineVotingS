    ...
    public class ResultController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<object>>> GetAllResults()
        {
            ...
            var response = results.Select(result => new
            {
                ...
                result.CandidateID,
                result.TotalVotes,
                Election = result.Elections != null ? new
                {
                    result.Elections.ElectionID,
                    result.Elections.StartDate,
                    result.Elections.EndDate,
                    result.Elections.CreatedAt,
                    result.Elections.UpdatedAt
                } : null,
                Candidate = result.Candidates != null ? new
                {
                    result.Candidates.CandidateID,
                    result.Candidates.ElectionID,
                    result.Candidates.FullName,
                    result.Candidates.Party,
                    result.Candidates.Description,
                    result.Candidates.Income,
        ...
        public async Task<ActionResult<object>> GetResultById([FromRoute] int resultId)
        {
            ...
            // Ensuring related entities are not null before accessing them
            var response = new
            {
                result.ResultID,
                result.ElectionID,
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<IEnumerable<object>>> GetResultsByCandidateId([FromRoute] int candidateId)
        {
            ...
            {
                return NotFound($"No results found for Candidate ID {candidateId}.");
            }
        ...
        public async Task<ActionResult<IEnumerable<Result>>> GetResultsByElectionId([FromRoute] int electionId)
        {
            ...
            if (results == null || !results.Any())
            {
                return NotFound($"No results found for Candidate ID {electionId}.");
            }
        ...
        public async Task<ActionResult<IEnumerable<Result>>> GetResultsByTotalVotesGreaterThan([FromRoute] int votes)
        {
            ...
            if (results == null || !results.Any())
            {
                return NotFound($"No results found for Candidate ID {votes}.");
            }
